...
public class LicenseManager
{
    ...
    public LicenseManager(string serverUrl)
    {
        ...
        api = new LicenseApiClient(serverUrl);

        CurrentSession = storage.LoadSession();
    }
    ...
    public async Task<LicenseActivationResult> ActivateAsync(string key)
    {
        ...
        ServerActivationResponse? result =
            await api.ActivateAsync(key.Trim());
                ...
                out LicenseRole role))
        {
            ...
            {
                Success = false,
                Message = "Server returned an invalid role."
            };
        }

        CurrentSession = new LicenseSession
        {
            Key = key.Trim(),
            Role = role,
            ActivatedAt = DateTime.UtcNow,
            ExpiresAt = result.ExpiresAt,
            IsLifetime = !result.ExpiresAt.HasValue
        };
    ...
    public async Task<LicenseValidationResult> ValidateAsync()
    {
        ...
        ServerValidationResponse? result =
            await api.ValidateAsync(CurrentSession.Key);
